...
class SBY_CPT {
	public function __construct() {
		add_action( 'admin_menu', array( $this, 'set_up_submenus' ) );
	...
	public static function set_up_submenus() {
		add_submenu_page(
			...
			'edit_' . SBY_CPT,
			'sby_single_settings',
			array( 'SBY_CPT', 'single_settings_admin_page' )
		);
		add_submenu_page(
			SBY_SLUG,
			__( 'All Videos', SBY_TEXT_DOMAIN ),
			__( 'All Videos', SBY_TEXT_DOMAIN ),
			'edit_' . SBY_CPT,
			'edit.php?post_type=' . SBY_CPT,
			null
		);
	}
	...
	public static function channel_action_listener() {
		...
		wp_safe_redirect( admin_url( 'admin.php?page=sby_single_settings' ) );
	}
