...
function sby_admin_init() {
	...
	$tabs = array(
		...
		array(
			'title' => __( 'License', SBY_TEXT_DOMAIN ),
			'slug' => 'license',
			'capability' => $capability,
			'numbered_tab' => false
		),
...
function sby_dismiss_api_key_notice() {

	update_user_meta( get_current_user_id(), 'sby_api_key_notice', 'dismissed' );

	die();
}
add_action( 'wp_ajax_sby_dismiss_api_key_notice', 'sby_dismiss_api_key_notice' );
...
function sby_do_import_batch() {
	...
	if ( $playlist ) {
		...
		sby_process_post_set_caching( $posts, 'sby_importer' );
...
add_action( 'wp_ajax_sby_do_import_batch', 'sby_do_import_batch' );
//
...
function sby_register_option() {
	// creates our settings in the options table
	register_setting('sby_license', 'sby_license_key', 'sby_sanitize_license' );
}
add_action('admin_init', 'sby_register_option');

function sby_sanitize_license( $new ) {
	$old = get_option( 'sby_license_key' );
	if( $old && $old != $new ) {
		delete_option( 'sby_license_status' ); // new license has been entered, so must reactivate
	}
	return $new;
}

function sby_activate_license() {

	// listen for our activate button to be clicked
	if( isset( $_POST['sby_license_activate'] ) ) {

		// run a quick security check
		if( ! check_admin_referer( 'sby_nonce', 'sby_nonce' ) )
...
add_action('admin_init', 'sby_activate_license');

function sby_deactivate_license() {

	// listen for our activate button to be clicked
	if( isset( $_POST['sby_license_deactivate'] ) ) {
...
add_action('admin_init', 'sby_deactivate_license');
...
add_action('admin_notices', 'sby_renew_license_notice');
function sby_renew_license_notice() {
    ...
    if( empty($sby_license) || !isset($sby_license) || ( get_user_meta($user_id, 'sby_ignore_notice') && $sby_notice_dismissible ) && !$sby_check_license ) return;
    ...
    if( $sby_license_expired || $sby_check_license ) {
        ...
        $sby_expired_box_msg .= " &nbsp;<a href='https://smashballoon.com/checkout/?edd_license_key=".$sby_license."&download_id=".$sby_download_id."&utm_source=plugin-pro&utm_campaign=sby&utm_medium=expired-notice-dashboard' target='_blank' class='button button-primary'>Renew License</a><a href='javascript:void(0);' id='sby-why-renew-show' onclick='sbyShowReasons()' class='button button-secondary'>Why renew?</a><a href='javascript:void(0);' id='sby-why-renew-hide' onclick='sbyHideReasons()' class='button button-secondary' style='display: none;'>Hide text</a> <a href='".$sby_url."' class='button button-secondary'>Re-check License</a></p>
            <div id='sby-why-renew' style='display: none;'>
                ...
                <p>Without a valid license key you will no longer be able to receive updates or support for the Custom YouTube Feed plugin. A renewed license key grants you access to our top-notch, quick and effective support for another full year.</p>
...
add_action('admin_init', 'sby_nag_ignore');
function sby_nag_ignore() {
    global $current_user;
        $user_id = $current_user->ID;
